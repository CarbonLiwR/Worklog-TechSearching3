...
<script setup lang="ts">
  ...
  const loadingMessage = { sender: 'bot', text: '你可以问我所有事情' };
  messages.value.push(loadingMessage)
  // 存储搜索结果的日志
  const logs = ref<
    { user_name: string; create_datetime: string; content: string }[]
  >([]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      messages.value[loadingMessageIndex] = { sender: 'bot', text: data.answer || '这是AI的默认回答。' };
    } catch (error) {
      // 处理错误，更新思考中的消息为错误提示
  ...
  onMounted(() => {
    if (query.value) {
      // message.value = query.value; // 将查询条件作为默认消息
      searchWorklogs(); // 发起搜索请求
      // sendMessage(); // 发送消息给 AI 接口
    }
  });
</script>
