      ...
      <a-card :title="$t('menu.admin.sysUser')" class="general-card">
        ...
        <div class="content">
          ...
          >
            <template #dept="{ record }">
              <a-popconfirm
                  :content="$t('admin.user.columns.updateUserDepts.placeholder')"
                  @ok="updateDept(record.username, record.id)"
              >
                <a-input-tag
                    :model-value="deptNameList(record.depts)"
                    :readonly="true"
                />
              </a-popconfirm>
            </template>
        ...
        <div class="content-modal">
          ...
          <a-modal
              :closable="false"
              :title="drawerTitle"
              :visible="openEditUserDept"
              @cancel="closeEditUserDept"
              @ok="submitUserDept"
          >
            <a-select
                v-model="formUserDept.depts"
                :allow-clear="true"
                :loading="loading"
                :multiple="true"
                :options="deptOptions"
...
import {
  ...
  SysUserRoleReq,
  updateUser,
  updateUserAvatar, updateUserDept,
  updateUserRole,
} from '@/api/user';
...
import {querySysDeptAllBySysUser, querySysDeptsAll, querySysDeptTree, SysDeptTreeParams} from '@/api/dept';
import {
  CasbinGroupDel,
  CasbinGroupReq,
  CasbinGroupsReq,
  createDeptCasbinGroup,
  createDeptCasbinGroups,
  createCasbinGroup,
  createCasbinGroups,
  deleteCasbinAllGroups, CasbinDeptGroupsReq, CasbinDeptGroupReq,
} from '@/api/casbin';
...
const formUserDept = reactive<SysUserDeptReq>({
  depts: [],
});
const filterUserDept = reactive<SysUserDeptReq>({
  depts: [],
});
const formUserRole = reactive<SysUserRoleReq>({
  roles: [],
});
const oldUserDept = reactive<SysUserDeptReq>({
  depts: [],
});
...
const userDeptData = ref<SysDeptRes[]>([]);
const deptOptions = computed<SelectOptionData[]>(() => {
  return userDeptData.value.map((item) => {
    return {
      value: item.id,
      label: item.name,
      disabled: Boolean(!item.status),
...
const updateDept = async (username: string, pk: number) => {
  operateUsername.value = username;
  drawerTitle.value = t('admin.user.columns.edit.dept');
  await fetchUser(username);
  await fetchUserDept(pk);
  await fetchAllDept();
  openEditUserDept.value = true;
};
...
const deptNameList = (deptList: SysDeptRes[]) => {
...
const openEditUserDept = ref<boolean>(false);
...
const closeEditUserDept = () => {
  formUserDept.depts = [];
  openEditUserDept.value = false;
};
...
const fetchAllDept = async () => {
  setLoading(true);
  try {
    userDeptData.value = await querySysDeptsAll();
  } catch (error) {
    // console.log(error);
  } finally {
    setLoading(false);
...
// 请求用户部门
const fetchUserDept = async (pk: number) => {
  setLoading(true);
  try {
    const res = await querySysDeptAllBySysUser(pk);
    formUserDept.depts = res.map((item) => item.id);
    oldUserDept.depts = formUserDept.depts;
  } catch (error) {
    // console.log(error);
  } finally {
    setLoading(false);
...
// 更新用户部门
const submitUserDept = async () => {
  setLoading(true);
  try {
    if (!listEqual(oldUserDept.depts, formUserDept.depts)) {
      if (oldUserDept.depts.length > 0) {
        await submitDeleteCasbinGroups({uuid: userUUID.value});
      }
      if (formUserDept.depts.length === 1) {
        await submitCreateDeptCasbinGroup({
          uuid: userUUID.value,
          dept: String(formUserDept.depts[0]),
        });
      } else if (formUserDept.depts.length > 1) {
        await submitCreateDeptCasbinGroups({
          gs: formUserDept.depts.map((item) => ({
            uuid: userUUID.value,
            dept: String(item),
          })),
        });
    ...
    console.log("开始更新用户部门");
    await updateUserDept(operateUsername.value, formUserDept);
    closeEditUserDept();
    await fetchUserList({
      page: pagination.current,
      size: pagination.pageSize,
...
// 创建 casbin group
const submitCreateDeptCasbinGroup = async (data: CasbinDeptGroupReq) => {
  setLoading(true);
  try {
    await createDeptCasbinGroup(data);
  } catch (error) {
    console.log(error);
  } finally {
    setLoading(false);
...
// 创建 casbin groups
const submitCreateDeptCasbinGroups = async (data: CasbinDeptGroupsReq) => {
  setLoading(true);
  try {
    await createDeptCasbinGroups(data);
  } catch (error) {
    // console.log(error);
  } finally {
    setLoading(false);
